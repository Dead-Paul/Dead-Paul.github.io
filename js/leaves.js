...
document.addEventListener("DOMContentLoaded", () => {
    let /**@type {Number}*/ wind = 1,
        /**@type {Number}*/ impulse = 1;

    const /**@type {HTMLElement} */ space = document.getElementById("animation-div") || document.documentElement,
        /**@type {Object} */ spaceParams = {
    ...
    function fallAnimation(img) {
        if (img.getAttribute("isBusy") === "true") return;
        let /**@type {Number} */ top =
                img.style.top && img.getAttribute("restart") !== "true"
                    ? parseInt(img.style.top, 10)
                    : Math.floor(Math.random() * 2) === 1
                    ? Math.floor(Math.random() * spaceParams.height()) - img.getBoundingClientRect().height
                    : 0,
            /**@type {Number} */ left =
                img.style.left && img.getAttribute("restart") !== "true"
        ...
        if (
            top <= spaceParams.height() - img.getBoundingClientRect().height &&
            left <= spaceParams.width() - img.getBoundingClientRect().width &&
            left >= 0
        ) {
            img.style.top = Math.floor(top) + "px";
            img.style.left = Math.floor(left) + "px";
        } else img.setAttribute("restart", "true");
    }

    /**@param {HTMLElement} img */
    function leafAnimation(img) {
        let /**@type {Number} */ rotate = img.style.rotate ? parseFloat(img.style.rotate) : Math.floor(Math.random() * 90);
